...
<#
.SYNOPSIS
    Create a GitHub work item (feature / task) for assimalign/cohesion-platforms in the shared
    org Project #13 "Cohesion", following the L04.01.NN WBS scheme, native parent/sub-issue
    links, and the Summary / Acceptance Criteria body template — classifying scope creep and
    reusing existing items instead of creating duplicates.
...
.EXAMPLE
    # Scope-creep TASK discovered on feature branch feature/L04.01.03.02-kubernetes-gateway-skeleton.
    ./New-CohesionWorkItem.ps1 -As task -Title "Validate kubeconfig context selection against in-cluster config" `
...
.EXAMPLE
    ...
    ./New-CohesionWorkItem.ps1 -As feature -Title "Implement image pull-secret rotation" -Wave W03
...
$areaName = $null
if ($areaEpic) {
    $am = [regex]::Match($areaEpic.title, 'Platforms - (.+)$')
