...
- **Dev image path:** when `IsLocal` (the developer-machine environment; `Development` is a deployable environment since cohesion item C1) and the selected context is Kind, resolve the resource's
