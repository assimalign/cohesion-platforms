...
settings, then parses Docker-specific arguments, and finally invokes the optional callback. The
...
| `--image-archive` | `--image-archive IMAGE=PATH` or `--image-archive=IMAGE=PATH` | Adds or replaces an `ImageArchives` mapping. The argument is repeatable and is split at its final `=`. |
