...
validated application image-index path and a target registry binding. During Gather, the gateway
resolves only a resource's own `ArtifactRef.Self`, keeps every reference digest-pinned, loads an
advertised archive into a Development Kind cluster, or returns an explicitly registry-bound
reference for other Kubernetes targets.
