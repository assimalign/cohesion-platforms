...
| Project | Purpose |
| --- | --- |
| `Assimalign.Cohesion.ApplicationModel.Gateway.Kubernetes` | `KubernetesGateway : ApplicationGateway` (`Name = "kubernetes"`), `KubernetesPlanCompiler`, plan controller, informer/observer, and `UseKubernetesGateway()`. Design item 34 is tracked by [`L04.01.03.09` / #31](https://github.com/assimalign/cohesion-platforms/issues/31). |
...
- The gateway can consume an already digest-pinned manifest image or an injected
  `IImageRealizer`. Design item 35 ([#32](https://github.com/assimalign/cohesion-platforms/issues/32))
  supplies the image index, registry, archive, and daemon-load acquisition paths; none of that
